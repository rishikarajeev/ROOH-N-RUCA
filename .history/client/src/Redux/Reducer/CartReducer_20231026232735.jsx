...
export const RemoveCartProducts = createAsyncThunk("cart/delete", async (_id) => {
  ...
  export const RemoveAllCartProducts = createAsyncThunk("cart/deleteALL", async (username) => {
    ...
    incrementProduct:(state,action)=>
      ...
      decrementProduct:(state,action)=>
           ...
           totalProducts:(state,action)=>
           {
             ...
             state.total = state.viewCart.reduce((acc, item) => acc + item.subtotal, 0);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getCartProducts.fulfilled, (state, action) => {
        state.status = "idle";
        console.log(JSON.stringifyaction.payload);
    ...
    builder
      ...
      builder
      .addCase(RemoveCartProducts.pending, (state, action) => {
        state.status = "loading";
      })
      ...
      builder
      .addCase(RemoveAllCartProducts.pending, (state, action) => {
        state.status = "loading";
      })
      .addCase(RemoveAllCartProducts.fulfilled, (state, action) => {
        state.viewCart = [];
        state.status = "fulfilled";
      })

      .addCase(RemoveAllCartProducts.rejected, (state, action) => {
        state.status = "error";
      });
  },
});
export default cartSlice.reducer;
export const {incrementProduct,decrementProduct,totalProducts}=cartSlice.actions;
