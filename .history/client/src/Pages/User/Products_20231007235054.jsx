...
function Products() {
  ...
  const [isFavorite, setIsFavorite] = useState({name:"",favourite:false});

  const toggleFavorite = (name) => {
      console.log("1",name);
      const itemIndex = productdata.findIndex((item) => item._id === name);
      ...
      if (itemIndex !== -1) {
        ...
        setIsFavorite({name:name,favourite:false});
        ...
        }else
        {
          ...
          updatedFavourite[itemIndex].username = username;
          setfavouriteData(updatedFavourite);
          setIsFavorite({name:name,favourite:true});
