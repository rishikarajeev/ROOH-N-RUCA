...
function Products() {
    ...
    const toggleFavorite = (name) => {
      const itemIndex = productdata.findIndex((name) => item.name === NamedNodeMap);
      const updatedFavourite=productdata.filter((item)=> item.name == name);
      const updateObjectValue = (objectId, newValue) => {
        setYourArray((productdata) =>
        productdata.map((item) =>
            item.name === objectId ? { ...item, value: newValue } : item
          )
        );
      };
      ...
      { 
        ...
        setfavouriteData(updatedFavourite[0]);
