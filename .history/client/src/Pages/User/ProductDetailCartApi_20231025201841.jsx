...
function PruductDetailCartApi() {
 ...
 const cartItems=  viewcartItems.filter((item)=>{
   return item.productid===id;
  });
  console.log(id);
  const newdata=const quantity = cartItems[0].quantity ? cartItems[0].product.quantity : undefined;
  ...
  useEffect(() => {
    ...
    axios.get(viewUrl).then((response) => {
      ...
      setProductata({
        ...
        productid: id,
        price: newdata.price,
        quantity: cartItems.quantity?cartItems.quantity:1,
        image: newdata.image,
              ...
              <h4 className="card-title fw-bolder">{productdata.name}</h4>
                ...
                <div className=" justify-content-md-start m-3 ">
                      ...
                      <div class="gap-2 d-md-flex justify-content-md-start border border-dark p-2   " style={{"max-width":"100px"}}>
                        ...
                        <span className="fs-5">{cartItems.quantity?cartItems.quantity:1}</span>
