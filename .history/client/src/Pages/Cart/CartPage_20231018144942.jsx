...
function CartPage() {
  ...
  const [cartUpdatedItems, setCartData] = useState([]);
  ...
  const checkout = () => {
    console.log("addd..", cartUpdatedItems);
    localStorage.setItem("cartData2", JSON.stringify(cartUpdatedItems));
  };
 ...
 setProductata({ ...productdata, quantity: quantity - 1,subtotal :calculateSubtotal(productdata(itemIndex)),total:calculateTotal()});
  ...
  const handleDecrement = (itemId, quantity) => {
    if (quantity > 1) {
      const itemIndex = cartUpdatedItems.findIndex(
      ...
      if (itemIndex !== -1) {
        // Create a new cart array with the updated quantity
        const updatedCart = [...cartUpdatedItems];
        // updatedCart[itemIndex].quantity -= 1;
        // updatedCart[itemIndex].subtotal = calculateSubtotal(
        //   updatedCart[itemIndex]
        // );
        calculateTotal();
        setCartData({ ...updatedCart, quantity: quantity - 1,subtotal :calculateSubtotal(productdata(itemIndex)),total:calculateTotal()});
