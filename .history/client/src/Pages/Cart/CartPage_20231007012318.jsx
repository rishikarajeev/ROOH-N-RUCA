  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const total = response.data.data.reduce((acc, item) => acc + (item.price*item.quantity), 0);
        console.log(total);
        setTotalPrice(total)
      ...
      <div className="container  bg-white border border-light">
        <h2>Shopping Cart</h2>
            ...
            {cartItems.map((item, i) => (
              <div className="card p-1 border border-white p-2" key={item._id}>
                <div className="card ">
                  <div className="row ">
                    <div className="col-sm-4 d-grid gap-2 d-md-flex justify-content-md-start  ">
                      <div className="row">
                        ...
                        <div className="col justify-content-md-start ">
                          <h4 className="card-title fs-small">{item.name}</h4>
                    ...
                    <div className="col-sm-8">
                     ...
                     <div className="row row-cols-3">
                        ...
                        <button className="col border border-white bg-white mx-auto fs-5 "  onClick={() => handleDecrement(item._id,item.quantity)}>
                          -
                        </button>
                        <span className="fs-5  text-center">{item.quantity}</span>
                        <button className=" border border-white bg-white mx-auto fs-5"   onClick={() => handleIncrement(item._id)}>
                          +
                        </button>
                        </div>
                        <div>
                          
                        <span className="fs-5 ms-3 text-start"><i className="fas fa-trash"></i></span>
                       </div>
                       <div>
