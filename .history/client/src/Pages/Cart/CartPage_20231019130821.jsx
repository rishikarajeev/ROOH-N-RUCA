...
function CartPage() {
  ...
  const calculateTotal = () => {
   ...
   // dispatch(incrementProduct({ _id: itemId, total:total }));
    console.log(total);
    setTotalPrice(total);
    // return total;
  };
  ...
  const handleIncrement = (itemId,item) => {
    ...
    dispatch(incrementProduct({ _id: itemId, quantity:item.quantity+ 1,subTotal:calculateSubtotal(item)}));
    //calculateSubtotal(item);
   // calculateSubtotal(item);
   // dispatch(subTotal({_id: itemId,quantity:item.quantity,price:item.price}));
    calculateTotal();
    const formData = new FormData();
    formData.append("username", username);
    formData.append("name", cartItems.name);
    formData.append("category", cartItems.category);
