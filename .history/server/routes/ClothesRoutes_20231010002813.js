...
ClothesRoute.get("/", checkAuth, (req, res) => {
  console.log(req.userData);
  ClothesModel.aggregate([
    {
      $lookup: {
        from: "favouriteSchema",
        localField: "_id",
        foreignField: "Clothes_id",
        as: "results",
      },
    },
    {
    ...
    },
    {
      $group: {
        ...
        category: { $first: "$results.phone" },
        price: { $first: "$results.location" },
        description: { $first: "$username" },
        image: { $first: "$username" },
        favourite: { $first: "$username" },
        favourite: { $first: "$username" },
      },
    },
  ]).then((data) => {
